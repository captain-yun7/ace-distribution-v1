...
if( $_GET[download] ) {
	...
	if($download==1){
		$fn = $row->bbs_file;
		$bbs_file = iconv("utf-8","euc-kr",$row->sbbs_file);
	} else if($download==2){
		$fn = $row->bbs_file2;
		$bbs_file = iconv("utf-8","euc-kr",$row->sbbs_file2);
	} else if($download==3){
		$fn = $row->bbs_file3;
		$bbs_file = iconv("utf-8","euc-kr",$row->sbbs_file3);
	} else if($download==4){
		$fn = $row->bbs_file4;
		$bbs_file = iconv("utf-8","euc-kr",$row->sbbs_file4);
	} else if($download==5){
		$fn = $row->bbs_file5;
		$bbs_file = iconv("utf-8","euc-kr",$row->sbbs_file5);
	}
