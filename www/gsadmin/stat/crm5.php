...
<div>
	<h3 class="page-header">접속로그분석</h3>
</div>
			...
			<table width="100%">
				<tr>
					<td> 
						...
						<button style="submit" class="btn btn-primary" title="검색"><span class="glyphicon glyphicon-search" aria-hidden="true"></span></button>&nbsp;&nbsp;&nbsp;<a href="<?=$_SERVER[PHP_SELF];?>?date=4" class="btn btn-primary">요일별</a> <a href="<?=$_SERVER[PHP_SELF];?>?date=5" class="btn btn-primary">IP별</a>
						<a href="<?=$_SERVER[PHP_SELF];?>?date=6" class="btn btn-primary">접속경로</a>
					</td>
					<td class="menu">전체 접속자 : <?=$total_connect_cnt;?> </td>
				</tr>
			</table>
		</div>
		</form>
	...
	<div class="table-responsive">
			...
			<caption></caption>
				...
				<tr>
					...
					<td>
						...
						<div class="progress">
							<div class="progress progress-bar progress-bar-info progress-bar-striped" role="progressbar"  aria-valuemax="100" style="width: <?=$total_cnt?>%"> </div>
						</div>
					</td>
		...
		<div class="table-responsive">
			...
			<?
				...
				// 전체 접속자 수
				$total_connect_cnt=$db->cnt("cs_connect", "");
				$result=$db->result("select count(ip) as CNT, ip from cs_connect group by ip order by CNT desc");
		...
		<div class="table-responsive">
			...
			<caption></caption>
				...
				<tr>
					...
					<td>
						<? $total_cnt = intval( $row[1] / $total_connect_cnt * 10000 ) / 100;?>
						<? $total_cnt_w = intval( $row[1] / $total_connect_cnt * 10000 ) / 100 * 3;?>
						<div class="progress">
							<div class="progress progress-bar progress-bar-info progress-bar-striped" role="progressbar"  aria-valuemax="100" style="width: <?=$total_cnt_w?>%"> </div>
						</div>
					</td>
					<td class="text-center"><?echo $total_cnt?>%</td>
				</tr>
			...
			<?
				...
				// 전체 접속자 수
				$total_connect_cnt=$db->cnt("cs_connect", "");
				$result=$db->result("select count(url) as CNT, url from cs_connect group by url order by CNT desc");
		...
		<div class="table-responsive">
			...
			<?
				...
				$result=$db->result("select substring(register, 6, 2), count(ip) from cs_connect where left(register, 4)='$_GET[year]' group by substring(register, 6, 2)");
		...
		<div class="table-responsive">
			...
			<?
				...
				$result=$db->result("select substring(register, 9, 2), count(ip) from cs_connect where left(register, 4)='$_GET[year]' and substring(register, 6, 2)=$_GET[mon] group by substring(register, 9, 2)");
		...
		<div class="table-responsive">
			...
			<?
				...
				$result=$db->result("select substring(register, 12, 2), count(ip) from cs_connect where left(register, 4)='$_GET[year]' and substring(register, 6, 2)=$_GET[mon] and substring(register, 9, 2)=$_GET[day] group by substring(register, 12, 2)");
		...
		<div class="table-responsive">
			...
			<!--table width="800" border="0" cellspacing="0" cellpadding="0" height="55">
				<tr>
					<td height="25" align="right"><a href="../../data/csv/crm5.csv"><img src="../images/bt_excell.gif" align="absmiddle" border="0"></a></td>
				</tr>
			</table-->
