...
if( $_POST[title_bar] ) {	
	...
	// 디비 입력
	$sql="kind='$_POST[kind]', display='$_POST[display]', start_day='$start_day', end_day='$end_day', width='$_POST[width]', height='$_POST[height]', tops='$_POST[tops]', lefts='$_POST[lefts]', live='$_POST[live]', title_bar='$_POST[title_bar]', link_url='$_POST[link_url]', popup_images='$popup_images', content='$_POST[content]' where idx='$_POST[idx]'";
	if( $db->update("cs_popup", $sql) ) { $tools->alertJavaGo('팝업창이 수정 되었습니다.', 'popup.php'); } else { @unlink("../../data/designImages/".$popup_images); $tools->errMsg('비상적으로 입력 되었습니다.');}
} else {
	$tools->errMsg('경 고 !!!\n\n비정상적으로 접근했습니다.');
}
?>
