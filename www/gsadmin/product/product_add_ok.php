...
if( $tools->chkDigit($_POST[part_code] )) {
	...
	if( $_POST[images1_check] == 1 && $_POST[images2_check] == 1 ) {
		...
		//} else {
			//@unlink($ROOT_DIR."/data/designImages/".$images1);
			//$tools->errMsg('등록할 이미지를 선택 하세요1');
			//exit();
		}
	} else {
		$tools->errMsg("등록할 이미지를 선택 하세요");
...
//파일 1-1
	if( $_FILES[file][size] > 0 ) {
		$EXT_CHECK = array("php", "php3", "htm", "html", "cgi", "perl");	// 업로드 파일 제한 확장자 추가 가능
		if( $EXT_TMP = explode( ".", $_FILES[file][name])) {
		...
		if( !@move_uploaded_file($_FILES[file][tmp_name], "../../data/upload/".$file) ) {
			$tools->errMsg("파일 업로드 에러");
		} else { @unlink($_FILES[file][tmp_name]);
		}
	} else {
		$file 	= "none";
	}
//파일 1-2
		...
		$EXT_CHECK = array("php", "php3", "htm", "html", "cgi", "perl");	// 업로드 파일 제한 확장자 추가 가능
		if( $EXT_TMP = explode( ".", $_FILES[file2][name])) {
			foreach ($EXT_CHECK as $value) {
				if( strstr( $value, strtolower($EXT_TMP[1]))) {
					$tools->errMsg( strtoupper($EXT_TMP[1])." 은 업로드 할수 없습니다." );
				}
		...
		if( $_FILES[file2][size]  > 1024*1024*4) {
			$tools->errMsg("업로드 용량 초과입니다\\n\\n4메가 까지 업로드 가능합니다"); exit();
		}
		$file2	= time()."&&".$_FILES[file2][name];
		if( !@move_uploaded_file($_FILES[file2][tmp_name], "../../data/upload/".$file2) ) {
			$tools->errMsg("파일 업로드 에러");
		} else { @unlink($_FILES[file2][tmp_name]);
		}
	} else {
		$file2 	= "none";
	}
...
//카달로그파일 2-1
		if( $_FILES[cdlog2_file1][size] > 0 ) {
		$EXT_CHECK = array("php", "php3", "htm", "html", "cgi", "perl");	// 업로드 파일 제한 확장자 추가 가능
		if( $EXT_TMP = explode( ".", $_FILES[cdlog2_file1][name])) {
			foreach ($EXT_CHECK as $value) {
				if( strstr( $value, strtolower($EXT_TMP[1]))) {
					$tools->errMsg( strtoupper($EXT_TMP[1])." 은 업로드 할수 없습니다." );
				}
		...
		if( $_FILES[cdlog2_file1][size]  > 1024*1024*4) {
			$tools->errMsg("업로드 용량 초과입니다\\n\\n4메가 까지 업로드 가능합니다"); exit();
		}
		$cdlog2_file1	= time()."&&".$_FILES[cdlog2_file1][name];
		if( !@move_uploaded_file($_FILES[cdlog2_file1][tmp_name], "../../data/upload/".$cdlog2_file1) ) {
			$tools->errMsg("파일 업로드 에러");
		} else { @unlink($_FILES[cdlog2_file1][tmp_name]);
		}
	} else {
		$cdlog2_file1 	= "none";
	}
	...
	//카달로그파일 2-2
		if( $_FILES[cdlog2_file2][size] > 0 ) {
		$EXT_CHECK = array("php", "php3", "htm", "html", "cgi", "perl");	// 업로드 파일 제한 확장자 추가 가능
		if( $EXT_TMP = explode( ".", $_FILES[cdlog2_file2][name])) {
			foreach ($EXT_CHECK as $value) {
				if( strstr( $value, strtolower($EXT_TMP[1]))) {
					$tools->errMsg( strtoupper($EXT_TMP[1])." 은 업로드 할수 없습니다." );
				}
		...
		if( $_FILES[cdlog2_file2][size]  > 1024*1024*4) {
			$tools->errMsg("업로드 용량 초과입니다\\n\\n4메가 까지 업로드 가능합니다"); exit();
		}
		$cdlog2_file2	= time()."&&".$_FILES[cdlog2_file2][name];
		if( !@move_uploaded_file($_FILES[cdlog2_file2][tmp_name], "../../data/upload/".$cdlog2_file2) ) {
			$tools->errMsg("파일 업로드 에러");
		} else { @unlink($_FILES[cdlog2_file2][tmp_name]);
		}
	} else {
		$cdlog2_file2 	= "none";
	}
...
//카달로그파일 3-1
		if( $_FILES[cdlog3_file1][size] > 0 ) {
		$EXT_CHECK = array("php", "php3", "htm", "html", "cgi", "perl");	// 업로드 파일 제한 확장자 추가 가능
		if( $EXT_TMP = explode( ".", $_FILES[cdlog3_file1][name])) {
			foreach ($EXT_CHECK as $value) {
				if( strstr( $value, strtolower($EXT_TMP[1]))) {
					$tools->errMsg( strtoupper($EXT_TMP[1])." 은 업로드 할수 없습니다." );
				}
		...
		if( $_FILES[cdlog3_file1][size]  > 1024*1024*4) {
			$tools->errMsg("업로드 용량 초과입니다\\n\\n4메가 까지 업로드 가능합니다"); exit();
		}
		$cdlog3_file1	= time()."&&".$_FILES[cdlog3_file1][name];
		if( !@move_uploaded_file($_FILES[cdlog3_file1][tmp_name], "../../data/upload/".$cdlog3_file1) ) {
			$tools->errMsg("파일 업로드 에러");
		} else { @unlink($_FILES[cdlog3_file1][tmp_name]);
		}
	} else {
		$cdlog3_file1 	= "none";
	}
	...
	//카달로그파일 3-2
		if( $_FILES[cdlog3_file2][size] > 0 ) {
		$EXT_CHECK = array("php", "php3", "htm", "html", "cgi", "perl");	// 업로드 파일 제한 확장자 추가 가능
		if( $EXT_TMP = explode( ".", $_FILES[cdlog3_file2][name])) {
			foreach ($EXT_CHECK as $value) {
				if( strstr( $value, strtolower($EXT_TMP[1]))) {
					$tools->errMsg( strtoupper($EXT_TMP[1])." 은 업로드 할수 없습니다." );
				}
		...
		if( $_FILES[cdlog3_file2][size]  > 1024*1024*4) {
			$tools->errMsg("업로드 용량 초과입니다\\n\\n4메가 까지 업로드 가능합니다"); exit();
		}
		$cdlog3_file2	= time()."&&".$_FILES[cdlog3_file2][name];
		if( !@move_uploaded_file($_FILES[cdlog3_file2][tmp_name], "../../data/upload/".$cdlog3_file2) ) {
			$tools->errMsg("파일 업로드 에러");
		} else { @unlink($_FILES[cdlog3_file2][tmp_name]);
		}
	} else {
		$cdlog3_file2 	= "none";
	}
...
//카달로그파일 4-1
		if( $_FILES[cdlog4_file1][size] > 0 ) {
		$EXT_CHECK = array("php", "php3", "htm", "html", "cgi", "perl");	// 업로드 파일 제한 확장자 추가 가능
		if( $EXT_TMP = explode( ".", $_FILES[cdlog4_file1][name])) {
			foreach ($EXT_CHECK as $value) {
				if( strstr( $value, strtolower($EXT_TMP[1]))) {
					$tools->errMsg( strtoupper($EXT_TMP[1])." 은 업로드 할수 없습니다." );
				}
		...
		if( $_FILES[cdlog4_file1][size]  > 1024*1024*4) {
			$tools->errMsg("업로드 용량 초과입니다\\n\\n4메가 까지 업로드 가능합니다"); exit();
		}
		$cdlog4_file1	= time()."&&".$_FILES[cdlog4_file1][name];
		if( !@move_uploaded_file($_FILES[cdlog4_file1][tmp_name], "../../data/upload/".$cdlog4_file1) ) {
			$tools->errMsg("파일 업로드 에러");
		} else { @unlink($_FILES[cdlog4_file1][tmp_name]);
		}
	} else {
		$cdlog4_file1 	= "none";
	}
...
//카달로그파일 4-2
		if( $_FILES[cdlog4_file2][size] > 0 ) {
		$EXT_CHECK = array("php", "php3", "htm", "html", "cgi", "perl");	// 업로드 파일 제한 확장자 추가 가능
		if( $EXT_TMP = explode( ".", $_FILES[cdlog4_file2][name])) {
			foreach ($EXT_CHECK as $value) {
				if( strstr( $value, strtolower($EXT_TMP[1]))) {
					$tools->errMsg( strtoupper($EXT_TMP[1])." 은 업로드 할수 없습니다." );
				}
		...
		if( $_FILES[cdlog4_file2][size]  > 1024*1024*4) {
			$tools->errMsg("업로드 용량 초과입니다\\n\\n4메가 까지 업로드 가능합니다"); exit();
		}
		$cdlog4_file2	= time()."&&".$_FILES[cdlog4_file2][name];
		if( !@move_uploaded_file($_FILES[cdlog4_file2][tmp_name], "../../data/upload/".$cdlog4_file2) ) {
			$tools->errMsg("파일 업로드 에러");
		} else { @unlink($_FILES[cdlog4_file2][tmp_name]);
		}
	} else {
		$cdlog4_file2 	= "none";
	}
...
//카달로그파일 5-1
		if( $_FILES[cdlog5_file1][size] > 0 ) {
		$EXT_CHECK = array("php", "php3", "htm", "html", "cgi", "perl");	// 업로드 파일 제한 확장자 추가 가능
		if( $EXT_TMP = explode( ".", $_FILES[cdlog5_file1][name])) {
			foreach ($EXT_CHECK as $value) {
				if( strstr( $value, strtolower($EXT_TMP[1]))) {
					$tools->errMsg( strtoupper($EXT_TMP[1])." 은 업로드 할수 없습니다." );
				}
		...
		if( $_FILES[cdlog5_file1][size]  > 1024*1024*4) {
			$tools->errMsg("업로드 용량 초과입니다\\n\\n4메가 까지 업로드 가능합니다"); exit();
		}
		$cdlog5_file1	= time()."&&".$_FILES[cdlog5_file1][name];
		if( !@move_uploaded_file($_FILES[cdlog5_file1][tmp_name], "../../data/upload/".$cdlog5_file1) ) {
			$tools->errMsg("파일 업로드 에러");
		} else { @unlink($_FILES[cdlog5_file1][tmp_name]);
		}
	} else {
		$cdlog5_file1 	= "none";
	}
...
//카달로그파일 5-2
		if( $_FILES[cdlog5_file2][size] > 0 ) {
		$EXT_CHECK = array("php", "php3", "htm", "html", "cgi", "perl");	// 업로드 파일 제한 확장자 추가 가능
		if( $EXT_TMP = explode( ".", $_FILES[cdlog5_file2][name])) {
			foreach ($EXT_CHECK as $value) {
				if( strstr( $value, strtolower($EXT_TMP[1]))) {
					$tools->errMsg( strtoupper($EXT_TMP[1])." 은 업로드 할수 없습니다." );
				}
		...
		if( $_FILES[cdlog5_file2][size]  > 1024*1024*4) {
			$tools->errMsg("업로드 용량 초과입니다\\n\\n4메가 까지 업로드 가능합니다"); exit();
		}
		$cdlog5_file2	= time()."&&".$_FILES[cdlog5_file2][name];
		if( !@move_uploaded_file($_FILES[cdlog5_file2][tmp_name], "../../data/upload/".$cdlog5_file2) ) {
			$tools->errMsg("파일 업로드 에러");
		} else { @unlink($_FILES[cdlog5_file2][tmp_name]);
		}
	} else {
		$cdlog5_file2 	= "none";
	}
	...
	$sql = "part_idx='$part_row->idx', display='$_POST[display]', code='$_POST[code]', name='$_POST[name]',company='$_POST[company]', youtong='$_POST[youtong]', bogan='$_POST[bogan]', boxip='$_POST[boxip]', old_price='$_POST[old_price]', shop_price='$_POST[shop_price]', unlimit='$_POST[unlimit]', number='$_POST[number]', point='$_POST[point]', box='$_POST[box]', option_check='$_POST[option_check]', option1_name='$_POST[option1_name]', option1_part='$_POST[hidden_option1_data]', option2_name='$_POST[option2_name]', option2_part='$_POST[hidden_option2_data]',images1='$images1', images2='$images2', add_images1='$add_images1', add_images2='$add_images2', add_images3='$add_images3', add_images4='$add_images4', add_images5='$add_images5', new_mark='$_POST[new_mark]', hit_mark='$_POST[hit_mark]', goods_file='$goods_file_name', tag='$_POST[tag]', content='$_POST[content]', register=now(), zzim='$_POST[zzim]'";

	if( $db->insert("cs_goods", $sql) ) { $tools->alertJavaGo("등록 하였습니다.","product_add.php"); } else { $tools->errMsg('비상적으로 입력 되었습니다.');}
