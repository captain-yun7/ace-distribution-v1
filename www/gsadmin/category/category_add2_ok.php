...
if( $_POST[part1_code] && $_POST[part2_code] ) {	
	...
	// 카테고리 순위 설정
	$part2_row = $db->row("cs_part", "where part_index = '$_POST[hidden_part_index]'", "max(part_ranking)");
	if( $part2_row[0] ) {	$part_ranking = $part2_row[0] +1; } else { $part_ranking = 1; }
	...
	// 디비 입력
	$sql="part_name='$_POST[part_name]', part1_code='$_POST[part1_code]', part2_code='$_POST[part2_code]', part_index='$_POST[hidden_part_index]', part_ranking='$part_ranking', goods_cnt='$_POST[goods_cnt]', part_display_check='$_POST[part_display_check]',  part_low_check='$_POST[part_low_check]', part_display_item='$_POST[part_display_item]', title_display_check='$_POST[title_display_check]', title_display_images='$title_display_images', title_html_code='$_POST[title_html_code]'";

	if( $db->insert("cs_part", $sql) ) { $tools->alertMetaGo("2차 카테고리 등록 되었습니다.", "category_list.php"); } else { @unlink("../../data/designImages/".$list_display_images1); @unlink("../../data/designImages/".$list_display_images2);  @unlink("../../data/designImages/".$title_display_images); $tools->errMsg('비상적으로 입력 되었습니다.');}
} else {
	$tools->errMsg('경 고 !!!\n\n비정상적으로 접근했습니다.');
}
?>
