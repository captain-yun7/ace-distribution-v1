...
if( $_POST[part1_code] && $_POST[part2_code] ) {	
	...
	$sql="part_name='$_POST[part_name]', goods_cnt='$_POST[goods_cnt]', part_display_check='$_POST[part_display_check]', part_low_check='$_POST[part_low_check]', part_display_item='$_POST[part_display_item]', title_display_check='$_POST[title_display_check]', title_display_images='$title_display_images', title_html_code='$_POST[title_html_code]' where idx='$_POST[idx]'";
	if( $db->update("cs_part", $sql) ) { $tools->alertMetaGo("2차 카테고리 수정 되었습니다.", "category_list.php"); } else { @unlink("../../data/designImages/".$title_display_images); $tools->errMsg('비상적으로 입력 되었습니다.');}
} else {
	$tools->errMsg('경 고 !!!\n\n비정상적으로 접근했습니다.');
}
?>
