...
class tools {
	...
	function filter($nameasdferwer){
		...
		//$nameasdferwer = mysql_real_escape_string($this->db_conn,$nameasdferwer); //SQL 명령문에 사용되는 문자열에서 특수 문자를 회피한다.
	...
	function filter2($nameasdferwer){
		...
		//$nameasdferwer = mysqli_real_escape_string($this->db_conn,$nameasdferwer);
	...
	$endata = @mcrypt_decrypt (MCRYPT_RIJNDAEL_128, $key, $data, MCRYPT_MODE_CBC, $IV);
	return $endata;
