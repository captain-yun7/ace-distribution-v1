    ...
    <>
      ...
      <main className="min-h-screen bg-[#FAF6F1]">
        ...
        <section className="py-20 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-16">
              <span className="text-sm font-medium text-[#B8956A] tracking-[0.3em] uppercase mb-4 block">CSR</span>
              <h2 className="text-3xl lg:text-4xl font-bold text-[#4A4039] mb-4">사회 공헌</h2>
              <p className="text-[#6B5D53]">나눔으로 함께하는 에이스유통</p>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-8">
              {csrActivities.map((activity, index) => (
                <div key={index} className="bg-gradient-to-br from-[#FAF6F1] to-white rounded-2xl p-8 text-center border border-[#E8DCC8] hover:border-[#B8956A] hover:shadow-xl transition-all duration-300 group">
                  ...
                  <h3 className="text-xl font-bold text-[#4A4039] mb-3">{activity.title}</h3>
                  <p className="text-[#6B5D53] mb-4">{activity.desc}</p>
                  <span className="inline-block bg-[#B8956A]/10 text-[#B8956A] font-semibold px-4 py-2 rounded-full text-sm">
                    {activity.stats}
                  </span>
                </div>
              ))}
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="space-y-4">
              {donationHistory.map((item, index) => (
                <div key={index} className="flex flex-col md:flex-row md:items-center gap-4 p-6 bg-white rounded-xl border border-[#E8DCC8]">
